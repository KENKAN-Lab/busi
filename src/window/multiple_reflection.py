...
def get_solar_spec_multi_layer(ss: List[SolarSpecSingleLayer], i: int, j: int) -> SolarSpecMultiLayer:
    """
    ...
    if i == j:
        ...
        # 層0から層j-1の日射特性値
        ss_ml = get_solar_spec_multi_layer(ss, i, j - 1)
...
def get_abs_multi_layer(ss: List[SolarSpecSingleLayer]) -> List[float]:
    """
    層iの日射吸収率を計算する。
    Args:
        ss: ガラス1枚に対する日射特性値
    ...
    # 層0から層jまでの日射特性値
    ss_ml_0_j = [get_solar_spec_multi_layer(ss, 0, j) for j in range(n)]

    # 層jから層n-1までの日射特性値
    ss_ml_j_n = [get_solar_spec_multi_layer(ss, j, n - 1) for j in range(n)]
    ...
    return [sol_f[j] * ss[j].a_f + sol_b[j] * ss[j].a_b for j in range(n)]


def get_total_solar_spec(ss: List[SolarSpecSingleLayer]) -> SolarSpecMultiLayer:
    """
    ...
    return get_solar_spec_multi_layer(ss, 0, len(ss) - 1)


if __name__ == '__main__':

    ss = [
        SolarSpecSingleLayer(tau_f=0.859, tau_b=0.859, rho_f=0.077, rho_b=0.077),
        SolarSpecSingleLayer(tau_f=0.859, tau_b=0.859, rho_f=0.077, rho_b=0.077),
        SolarSpecSingleLayer(tau_f=0.859, tau_b=0.859, rho_f=0.077, rho_b=0.077)
    ]

    rho = get_total_solar_spec(ss).rho_f
    tau = get_total_solar_spec(ss).tau_f

    alpha = get_abs_multi_layer(ss)

    # グレージング複合体全体の正面側の反射率・グレージング複合体全体の透過率・層jの日射吸収率の合計
    total = rho + tau + sum(alpha)

    print('τ + ρ + α = ' + str(total))
    print('ρ = ' + str(rho))
    print('τ = ' + str(tau))
    print('α1 = ' + str(alpha[0]))
    print('α2 = ' + str(alpha[1]))
    print('α3 = ' + str(alpha[2]))
